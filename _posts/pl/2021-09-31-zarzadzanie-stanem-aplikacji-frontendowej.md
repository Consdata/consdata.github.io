...
* Akcje są przechwytywane przez **reducery** (ang. _reducers_). Reducerem nazywamy czystą (deterministyczną oraz nieposiadającą efektów ubocznych) funkcję, która konsumuje akcję i, w zależności od jej przeznaczenia oraz zgodnie z logiką reducera, zwraca odpowiednio zmodyfikowany, nowy store.
